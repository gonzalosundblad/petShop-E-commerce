...
import {getAllReviewsRequest, postReviewRequest, putReview, deleteReviewRequest} from '../Redux/actionsReview';
import {connect} from 'react-redux';
import { bindActionCreators } from 'redux';
import axios from 'axios';

function Reviews({id, user, reviews, getAllReviewsRequest, postReviewRequest}){
...
//console.log(usuario.role);
    ...
    var list = [{uno: [], dos : [], tres : [], cuatro : [], cinco : []}]
      ...
      function onSend(){
        if(user.isLoggedIn && user.user.user.user_id){
      ...
      return (
        <div className={Style.box}>
          <div >
                      ...
                      reviews && reviews.map(o => {
                        return (
                          <div onSubmit={handleSubmit}>
                            <div value={o.review_id} onChange={handleChange}>
            ...
            {user.isLoggedIn ? <button name="modificar" onClick={onPut} className={Style.bottton} type="submit" value={o.review_id}>
                                Modificar comentario
                  </button> : null }
          {user.isLoggedIn ? <button name="eliminar" onClick={onDelete} className={Style.bottton} type="submit" value={o.review_id}>
                                Eliminar comentario
                  </button> : null }
                            </div>
                            <p className={Style.opinionsDate}>{o.createdAt.slice(0, 10)}</p>
                    <p className={Style.opinionsTitle}>{o.user.name }{o.user.last_name }</p>
...
{user.isLoggedIn ?
              <button name="enviar" onClick={onSend} className={Style.bottton} type="submit">
                      Enviar comentario
                  </button> : <p> Para dejar tu opinion tenes que estar logueado </p>}
...
const mapDispatchToProps = dispatch => {
  return {
    dispatch,
    ...bindActionCreators({getAllReviewsRequest, postReviewRequest, deleteReviewRequest}, dispatch)
  }
}
