...
export function getProducts() { //obtener todos los productos
    const request = axios.get('http://localhost:3001/products')                                   //ya esta
    return { type:GET_PRODUCTS, payload: request}; }


export function getCategories() { //obtener todas las categorias
    const request = axios.get('http://localhost:3001/products/category');                         //ya esta
...
export function getProductByCategory(name){//obtener todos los productos de una categoria
    const request = axios.get(`http://localhost:3001/products/category/${name}`)                  //ya esta
    return { type: GET_CATEGORIES_NOMBRECAT, payload : request }; }

export function getProductById(id){
    const request = axios.get(`http://localhost:3001/products/${id}`)
    return { type: GET_ID, payload : request } ; }//obtener un producto segun su id                //ya esta
// HASTA ACA FUNCIONA--------------------------
//----------------------------------------------
...
export function postCategory(data){   //agrega una categoria
  const request = axios.post('http://localhost:3001/products/category', data)                      //ya esta
  return { type: POST_CATEGORY, payload : request } ; }
//----------------------------------------
// export function putId(id){   //modifica un producto segun id
//   const request = axios.put(`http://localhost:3001/products/${id}`)
//   console.log(request);
//   return { type: PUT_ID, payload : request } ; }
export function putId (id, cambios) {
  return (dispatch) => {
     dispatch({type: PUT_ID});
...
export function deleteCategory(id){   //elimina una categoria
const request = axios.delete(`http://localhost:3001/products/category/${id}`)                    //anda con el cors
return { type: DELETE_CATEGORY_ID, payload : request } ; }

export function search(producto){   //busca entre todo
const request = axios.get(`http://localhost:3001/search?products=${producto}`)                   //no me anda
